...
const checkHouse = async () => {
    ...
    const reg = /^([\s]*[a-zA-Z0-9]{1,3}[\s]*)([\s])*([\-\/]{0,1}[\s]*)([\s])*([\s]*[a-zA-Z0-9]{0,3}[\s]*)$/;
...
const postData = async () => {
    ...
    try {
        ...
        const emVal = document.getElementById("email").value;
        ...
        const url = 'https://localhost:7182/Doctor';
        const params = {
            method: 'POST',
...
const load = async () => {
    ...
    //     let x = data.data;
    //     let i=0;
    //     document.getElementById("fname").value = x[i].first_name;
    //     document.getElementById("lname").value = x[i].last_name;
